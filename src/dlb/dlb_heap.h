...
#define dlb_heap__left(index) ((index) * 2)
#define dlb_heap__right(index) ((index) * 2 + 1)
...
{
    size_t left = dlb_heap__left(index);
    size_t right = dlb_heap__right(index);
    while (left || right)
    {
        if (heap->nodes[left].priority >= heap->nodes[right].priority)
        {
            dlb_heap__swap_nodes(heap, index, left);
            index = left;
        }
        else
        {
            dlb_heap__swap_nodes(heap, index, right);
            index = right;
        }
        left = dlb_heap__left(index);
        right = dlb_heap__right(index);
    }
}
